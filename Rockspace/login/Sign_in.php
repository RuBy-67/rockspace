...
<head>
  ...
  <link rel="stylesheet" href="../login/Sign_in.css">
...
<body>
  ...
  <main>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if ($result === false) {
        echo '<p class="error">Erreur lors de l`exécution de la requête : </p>' . $conn->error;
      }
      if ($result->num_rows == 0) {
        echo '<p class="error">Identifiants incorrects</p>.';
      }

      // Récupération du mot de passe stocké dans la base de données pour cet utilisateur
      ...
      if (password_verify($password, $stored_password)) {
        ...
        echo '<p class="green">Vous êtes maintenant connecté.</p>';
        // Redirection vers la page d'accueil
        header("Location: ../home/home.php");
        exit();
      } else {
        echo '<p class="error">Mdp incorrects.</p>';
      }

    }
    ?>
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
      <input class="champ" type="text" name="login" required placeholder="Login"><br><br>
      <input class="champ" type="password" name="mdp" required placeholder="Mots de passe"><br><br>
